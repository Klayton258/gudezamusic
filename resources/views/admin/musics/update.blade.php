            ...
            @endif
                ...
                <div class="bg-secondary rounded p-4 p-sm-5 my-4 mx-3">
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        <a href="{{ route('deletemusic', ['id'=>$music->id]) }}" class="">
                            <h3 class="" style="color: #c81313 !important;"><i class="fa fa-trash me-2"></i>Apagar</h3>
                        </a>
                        <h3>Atualizar Musica #{{$music->id}}</h3>
                    </div>
                    <form action="{{ route('updatemusic', ['id'=>$music->id]) }}" enctype="multipart/form-data" method="POST">
                        @csrf
                        <div class="form-floating mb-3">
                            <input type="file" class="form-control" name="m_cover" onchange="readURL(this);" id="floatingText" placeholder="image" accept="image/jpg, image/png, image/jpeg">
                            <label for="floatingText">Imagem</label>
                        </div>
                        <div class="form-floating mb-3">
                            <input type="text" class="form-control" name="m_title" id="floatingText" value="{{$music->m_title}}" required>
                            <label for="floatingText">Titulo</label>
                        </div>
                        <div class="form-floating mb-3">
                            <input type="text" class="form-control" name="m_artist" id="floatingInput" value="{{$music->m_artist}}" required>
                            <label for="floatingInput">Artista</label>
                        </div>
                        <div class="form-floating mb-4">
                            <input type="text" class="form-control" name="youtube" id="floatingPassword" value="{{$music->youtube}}">
                            <label for="floatingPassword">Youtube</label>
                        </div>
                        <div class="form-floating mb-4">
                            <input type="text" class="form-control" name="audiomack" id="floatingPassword" value="{{$music->audiomack}}">
                            <label for="floatingPassword">Audiomack</label>
                        </div>
                        <div class="form-floating mb-4">
                            <input type="text" class="form-control" name="soundcloud" id="floatingPassword" value="{{$music->soundcloud}}">
